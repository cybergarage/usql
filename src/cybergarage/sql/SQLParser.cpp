...
struct SQLGrammar : public boost::spirit::grammar<SQLGrammar>
{
	template <typename ScannerT> struct definition
	{
		typedef rule<ScannerT> SQLRule;

		rule<ScannerT> SQLTerminalCharacter;
		rule<ScannerT> SQLLanguageCharacter;
		rule<ScannerT> SimpleLatinLetter;
		rule<ScannerT> SimpleLatinUpperCaseLetter;
		rule<ScannerT> SimpleLatinLowerCaseLetter;
		rule<ScannerT> Digit;

		definition(const SQLGrammar& self)
		{
...
/*
			...
			DelimitedIdentifier ::= DoubleQuote <delimited identifier body> DoubleQuote
*/
			/*
			...
			*/
			//QualifiedName ::= /* [ <schema name> <period> ] */ QualifiedIdentifier
			/*
...
/*
			QuerySpecification ::=
					str_p(SELECT) [ <set quantifier> ] SelectList TableExpression
			SelectList ::=
					Asterisk
*/
					/*
...
//			FromClause ::= str_p(FROM) TableReference /* [ { <comma> <table reference> }... ] */
			...
			*/
			...
			/*
			TableReference ::=
				    TableName [ <correlation specification> ]
...
*/
		...
		const rule<ScannerT>& start() const {return Digit;}
	};
...
bool SQLParser::parse(const std::string &queryString)
{/*

	SQLGrammar sqlGrammar;
    string str;

	parse_info<> r1 = boost::spirit::parse(str.c_str(), sqlGrammar, space_p);
	boost::spirit::parse(queryString.begin(), queryString.end(), sqlGrammar);

	calculator calc;
	parse_info<> r2 = boost::spirit::parse(str.c_str(), calc, space_p);
*/
	return false;
}
